    ...
    this.startButton.onclick = function() {
      ...
      for (var i = 0, il = frames.length; i < il; i++) {
        ...
        for (var j = 0, jl = scripts.length; j < jl; j++) {
          if (scripts[j].src.indexOf('soundcOnes.js') !== -1) {
            reload = true;
            break;
          }
        }
...
Demo.prototype = {
  ...
  createSoundCone : function(object, innerAngle, outerAngle, outerGain) {
    ...
    var innerCone = new THREE.Mesh(
      innerConeGeo,
      new THREE.MeshBasicMaterial({color: 0x80FFE5, opacity: 0.5})
    );
    ...
    var outerCone = new THREE.Mesh(
      outerConeGeo,
      new THREE.MeshBasicMaterial({color: 0xFF9090, opacity: 0.5})
    );
  ...
  setupObjects : function() {
    ...
    for(var i = 0; i < this.numberOfCubes; i++){
      this.cubes.push(new THREE.Mesh());
    ...
    for(var i = 0; i < this.numberOfCubes; i++){
        this.cubes[i].sound = this.loadSound('samples/beat.wav');
        if (this.orientationEnabled) {
          this.cubes[i].rotation.x = 2*Math.PI/this.numberOfCubes*i;
          this.createSoundCone(this.cubes[i], 0.5, 2.0, 0.05);
        }
    }
  ...
  update : function(t, dt) {
    ...
    for( var i = 0; i < this.numberOfCubes; i++){
      this.setPosition(this.cubes[i], 0, 0, 0, dt/1000);
      if (this.velocityEnabled && this.orientationEnabled) {
        this.cubes[i].rotation.x += dt/1000;
      }
      //this.cubes[i].rotation.y += dt/800;
    }
